...
int main(void) {
    ...
    GPIOPinTypeGPIOOutput(GPIO_PORTF_BASE, 0x01 | 0x10);
    while(1) {
        // Encender ambos LEDs [cite: 174, 190]
        GPIOPinWrite(GPIO_PORTN_BASE, 0x01 | 0x02, 0x01 );
        GPIOPinWrite(GPIO_PORTF_BASE, 0x01 | 0x10, 0x01 );

        // Delay aumentado para que sea visible 
        for(ui32Loop = 0; ui32Loop < 5000000; ui32Loop++) { }

        //
        GPIOPinWrite(GPIO_PORTN_BASE, 0x01 | 0x02, 0x02 | 0x01);
        GPIOPinWrite(GPIO_PORTF_BASE, 0x01 | 0x10, 0x01 | 0x10);
        ...
        GPIOPinWrite(GPIO_PORTF_BASE, 0x01 | 0x10, 0x00 );
